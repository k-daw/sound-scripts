...
class AudioChannelRouter:
    def __init__(self):
        ...
        self.input_gain_db = 0  # Input gain adjustment in dB
        self.output_gain_db = 0  # Output gain adjustment in dB
    ...
    def get_channel_levels(self, audio_data):
        """Calculate RMS level for each channel (for display only)"""
        levels = []
        # Apply input gain for level display
    ...
    def start_recording(self, filename=None):
        ...
        self.recorded_frames = []
        print(f"\n✓ Recording to file: {filename}")
        return filename
    
    def stop_recording(self):
        """Stop recording to WAV file"""
        if self.wav_file:
    ...
    def process_audio_blocking(self, input_device_idx, output_device_idx):
        ...
        try:
            ...
            while self.running:
                ...
                # Apply input gain to the actual audio data
                ...
                if active_ch is not None:
                    ...
                    # Extract active channel data (use the gained input)
                    ...
                    status_text = f"\rActive: Ch{active_ch + 1} ({levels[active_ch]:>5.1f} dB) | {level_display}"
                ...
                if self.record_to_file:
                    ...
                    # Debug: show we're recording
                    if len(self.recorded_frames) % 100 == 0:
                        print(f" [{len(self.recorded_frames)} frames]", end="", flush=True)
    ...
    def start(self, input_device_idx, output_device_idx):
        ...
        try:
            ...
            # Ask about recording to file
            record_file = input("\nRecord to WAV file for testing? (y/n, default=y): ").lower()
            if record_file != 'n':
            ...
            print("Make some noise into your microphones!\n")
...
def main():
    ...
    if configure == 'y':
        print("\nCurrent settings:")
        print(f"  Input Gain: {router.input_gain_db:+.1f} dB")
        print(f"  Output Gain: {router.output_gain_db:+.1f} dB")
        ...
        input_gain = input(f"Input gain in dB (press Enter to keep {router.input_gain_db:+.1f}): ")
        if input_gain:
            router.input_gain_db = float(input_gain)
        
        output_gain = input(f"Output gain in dB (press Enter to keep {router.output_gain_db:+.1f}): ")
        if output_gain:
            router.output_gain_db = float(output_gain)
